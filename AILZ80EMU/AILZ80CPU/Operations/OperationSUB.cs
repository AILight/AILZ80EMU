...
{
    public class OperationSUB : OperationItem
    {
        ...
        {
            { @"A", (cpu) => { cpu.Register.SUB_8(RegisterEnum.A); } },
            { @"B", (cpu) => { cpu.Register.SUB_8(RegisterEnum.B); } },
            { @"C", (cpu) => { cpu.Register.SUB_8(RegisterEnum.C); } },
            { @"D", (cpu) => { cpu.Register.SUB_8(RegisterEnum.D); } },
            { @"E", (cpu) => { cpu.Register.SUB_8(RegisterEnum.E); } },
            { @"H", (cpu) => { cpu.Register.SUB_8(RegisterEnum.H); } },
            { @"L", (cpu) => { cpu.Register.SUB_8(RegisterEnum.L); } },
        ...
        {
            { @"n", (cpu) => { cpu.Register.Internal_8bit_Register = cpu.Bus.Data; cpu.Register.SUB_8(RegisterEnum.Internal_8bit_Register); } },
        };

        private OperationSUB(InstructionItem instructionItem)
            : base(instructionItem)
        {
        }

        public static new OperationSUB Create(InstructionItem instructionItem)
        {
            if (instructionItem.OpCode != OpCodeEnum.SUB)
            {
                return default!;
            }
            
            var executer = default(Action<CPUZ80>);
            var operationItem = new OperationSUB(instructionItem);
            ...
            else if (instructionItem.Operand == "(HL)")
            {
                ...
                operationItem.ExecuterForRead = (cpu) =>
                {
                    ...
                    cpu.Register.SUB_8(RegisterEnum.Internal_8bit_Register);
                };
