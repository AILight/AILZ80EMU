...
{
    public class OperationOR : OperationItem
    {
        ...
        {
            { @"A", (cpu) => { cpu.Register.OR_8(RegisterEnum.A); } },
            { @"B", (cpu) => { cpu.Register.OR_8(RegisterEnum.B); } },
            { @"C", (cpu) => { cpu.Register.OR_8(RegisterEnum.C); } },
            { @"D", (cpu) => { cpu.Register.OR_8(RegisterEnum.D); } },
            { @"E", (cpu) => { cpu.Register.OR_8(RegisterEnum.E); } },
            { @"H", (cpu) => { cpu.Register.OR_8(RegisterEnum.H); } },
            { @"L", (cpu) => { cpu.Register.OR_8(RegisterEnum.L); } },
        ...
        {
            { @"n", (cpu) => { cpu.Register.Internal_8bit_Register = cpu.Bus.Data; cpu.Register.OR_8(RegisterEnum.Internal_8bit_Register); } },
        };

        private OperationOR(InstructionItem instructionItem)
            : base(instructionItem)
        {
        }

        public static new OperationOR Create(InstructionItem instructionItem)
        {
            if (instructionItem.OpCode != OpCodeEnum.OR)
            {
                return default!;
            }
            
            var executer = default(Action<CPUZ80>);
            var operationItem = new OperationOR(instructionItem);
            ...
            else if (instructionItem.Operand == "(HL)")
            {
                ...
                operationItem.ExecuterForRead = (cpu) =>
                {
                    ...
                    cpu.Register.OR_8(RegisterEnum.Internal_8bit_Register);
                };
