...
{
    public class OperationAND : OperationItem
    {
        ...
        {
            { @"A", (cpu) => { cpu.Register.AND_8(RegisterEnum.A); } },
            { @"B", (cpu) => { cpu.Register.AND_8(RegisterEnum.B); } },
            { @"C", (cpu) => { cpu.Register.AND_8(RegisterEnum.C); } },
            { @"D", (cpu) => { cpu.Register.AND_8(RegisterEnum.D); } },
            { @"E", (cpu) => { cpu.Register.AND_8(RegisterEnum.E); } },
            { @"H", (cpu) => { cpu.Register.AND_8(RegisterEnum.H); } },
            { @"L", (cpu) => { cpu.Register.AND_8(RegisterEnum.L); } },
        ...
        {
            { @"n", (cpu) => { cpu.Register.Internal_8bit_Register = cpu.Bus.Data; cpu.Register.AND_8(RegisterEnum.Internal_8bit_Register); } },
        };

        private OperationAND(InstructionItem instructionItem)
            : base(instructionItem)
        {
        }

        public static new OperationAND Create(InstructionItem instructionItem)
        {
            if (instructionItem.OpCode != OpCodeEnum.AND)
            {
                return default!;
            }
            
            var executer = default(Action<CPUZ80>);
            var operationItem = new OperationAND(instructionItem);
            ...
            else if (instructionItem.Operand == "(HL)")
            {
                ...
                operationItem.ExecuterForRead = (cpu) =>
                {
                    ...
                    cpu.Register.AND_8(RegisterEnum.Internal_8bit_Register);
                };
